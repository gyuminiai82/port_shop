...
export default function CancelOrderButton({ orderId }: { orderId: string }) {
  ...
  const executeCancel = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setAlertMessage(data.error || "주문 취소에 실패했습니다.");
      } else {
      ...
      {showConfirm && (
        <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ background: "white", padding: "2rem", borderRadius: "16px", maxWidth: "400px", width: "90%", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.1)" }}>
            <h3 style={{ fontSize: "1.25rem", fontWeight: 800, marginBottom: "1rem", color: "#1f2937" }}>주문 취소 확인</h3>
            <p style={{ color: "#4b5563", marginBottom: "2rem", lineHeight: 1.5 }}>정말로 이 주문을 취소하시겠습니까? 취소 후에는 복구할 수 없습니다.</p>

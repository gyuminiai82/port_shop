...
export default function InquiryListClient({ inquiries, currentPage, totalPages }: { inquiries: Inquiry[], currentPage: number, totalPages: number }) {
      ...
      <div className="glass" style={{ background: "white", padding: "1.5rem", borderRadius: "24px", boxShadow: "0 10px 40px rgba(0,0,0,0.05)" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "800px", textAlign: "left" }}>
            <thead>
              <tr style={{ borderBottom: "2px solid #f3f4f6", color: "var(--text-secondary)" }}>
                ...
                <th style={{ padding: "1rem", fontWeight: 600, whiteSpace: "nowrap" }}>작성자</th>
              ...
              {inquiries.length === 0 ? (
                ...
                inquiries.map((inquiry) => (
                  <tr key={inquiry.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                    ...
                    <td style={{ padding: "1rem", whiteSpace: "nowrap" }}>
                      <div style={{ fontWeight: 600 }}>{inquiry.user?.name || "이름 없음"}</div>
                      <div style={{ fontSize: "0.875rem", color: "var(--text-secondary)" }}>{inquiry.user?.email}</div>
                    </td>
        ...
        {totalPages > 1 && (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "0.5rem", marginTop: "2rem" }}>
            <button 
              onClick={() => router.push(`/admin/inquiries?page=${Math.max(1, currentPage - 1)}`)}
            ...
            <div style={{ display: "flex", gap: "0.25rem" }}>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(pageNum => (
                <button
                  key={pageNum}
                  onClick={() => router.push(`/admin/inquiries?page=${pageNum}`)}
                  style={{
            ...
            <button 
              onClick={() => router.push(`/admin/inquiries?page=${Math.min(totalPages, currentPage + 1)}`)}
